...
class TensorProductDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, *tensors: Tensor) -> None:
        self.lengths = itertools.product(*[np.arange(len(tensor)) for tensor in tensors])
        self.tensors = itertools.product(*tensors)
    
    def __iter__(self):
        return iter(zip(self.lengths, self.tensors))
...
class BinarizedBinaryClassifier(FcaClassifier):
    ...
    def compute_support(self, test, device=torch.device("cuda")):
        
        test, context = (torch.tensor(arr, device=device, dtype=float) for arr in (test, self.context))
        
        train_pos = context[self.labels == True]
        train_neg = context[self.labels == False]
        ...
        data = torch.utils.data.TensorDataset(test)
        
        for i, (x,) in enumerate(data):
            intsec_pos = x.reshape(1, -1) * train_pos
            n_support_pos = ((intsec_pos @ (1 - train_pos.T)) == 0).sum(axis=1)
            n_counter_pos = ((intsec_pos @ (1 - train_neg.T)) == 0).sum(axis=1)

            intsec_neg = x.reshape(1, -1) * train_neg
            n_support_neg = ((intsec_neg @ (1 - train_neg.T)) == 0).sum(axis=1)
            n_counter_neg = ((intsec_neg @ (1 - train_pos.T)) == 0).sum(axis=1)

            positive_support[i] = n_support_pos.cpu().numpy()
            positive_counter[i] = n_counter_pos.cpu().numpy()
            negative_support[i] = n_support_neg.cpu().numpy()
            negative_counter[i] = n_counter_neg.cpu().numpy()
