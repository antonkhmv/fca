...
def create_binary_data_config(trial):
    ...
    if n_cuts is not None:
        qcut_cols = dict.fromkeys(students_int_cols, n_cuts)
...
def create_pattern_data_config(trial):
    ...
    if trial.suggest_int("onehot_encode_cats", 0, 1):
        onehot_cols.extend(students_cat_cols)
    else:
        categorical.extend(students_cat_cols)
    ...
    return data_config, categorical
